...
V20190513a:1.实现每包的层数限制和每层的块数限制；2.实现用最低水平线算法来排样
未解决问题：1.长条子需要单独打包，短或较短的条子可以进行拼接；2.重组时，可能会将一个非常长的条子，如（30,2436）放到目标区域中
           3.一包有最大层数限制，比如6层，但此处没有(已解决); 4.22O21S23,31O1S1,36O1S1,36O1S2四个组件未能成功打包，
           需要考虑层数及块数限制(已解决)
           4.放到区域中的部件总重量可能会大于50kg
           5.w or h < = 100的条子需要单独处理
...
class Package(object):
    def __init__(self, log=None):
        # self.start = time.time()  # 该类调用执行开始时间
        self.log = log
        ...
        # 计时间隔interval=1000ms,打开计时器timer_second,计时器从0开始计时到1000ms，然后计时器timer_second通过wx.PyTimer(MethodName)
        # 调用MethodName方法，继续计时再计时1000ms，再次调用MethodName方法，实现每隔1秒钟调用一下MethodName方法
...
# *****************************************************底层方法**********************************************************
    ...
    def spilt_package(self, key, value):
        ...
        for i in range(len(new_nums) - 1):
            self.info_box_part_dicts[key + '-' + str(i + 1)] = value[new_nums[i]: new_nums[i] + new_nums[i + 1]]  # 加入拆分后的新包
            ...
            for package in info_package_plies_plate:  # 对组件下每一包进行循环，一包对应work_package_task_list表单一条记录
                ...
                for i in range(total_plies):  # 循环层数
                    ...
                    for j in range(same_count[i + 1]):  # 循环层下面的块数
                        ...
                        seq = (str(element_info[j][0]), str(element_info[j][1]), str(element_info[j][4]), str(element_info[j][5]), str(element_info[j][7]), str(element_info[j][-1]))
...
def get_time():
    return time.strftime("%Y年%m月%d日 %H:%M:%S  ", time.localtime(time.time()))
...
class MyApp(wx.App, wx.lib.mixins.inspection.InspectionMixin):
    def OnInit(self):
        mainframe = MyFrame(parent=None, id=wx.ID_ANY, title="自动分包")
...
class MyFrame(wx.Frame):
    ...
    def CreateTextCtrl(self, ctrl_text=""):
        ...
        if ctrl_text.strip():
            ...
            text = ""
        return MyLogCtrl.WriteLog(self.panel, -1, text)
